...
from flyenemy import FlyingEnemy
...
class LevelWindow3:
    def __init__(self, screen, clock, gif_path, selected_character):
        ...
        self.enemies = [
            Enemy(self.start_x + 3 * self.tile_size, self.start_y + 9 * self.tile_size, "assets/enemy.png", self.tile_size),
            Enemy(self.start_x + 5 * self.tile_size, self.start_y + 5 * self.tile_size, "assets/enemy.png", self.tile_size),
            Enemy(self.start_x + 11 * self.tile_size, self.start_y + 7 * self.tile_size, "assets/enemy.png", self.tile_size)
        ]

        self.flying_enemy = FlyingEnemy(self.start_x + 6 * self.tile_size, self.start_y + 2 * self.tile_size, "assets/fly.png", self.tile_size)
    ...
    def show_victory_screen(self):
        ...
        msg = font_big.render("¡Nivel 3 completado!", True, (0, 255, 0))
    ...
    def run(self):
        while self.running:
            ...
            for row_idx, row in enumerate(self.level_map):
                for col_idx, tile in enumerate(row):
                    if tile == 'E':
                        ...
                        if door_rect.colliderect(self.player.get_rect()):
            ...
            for explosion in self.explosions[:]:
                ...
                elif explosion.get_rect().colliderect(self.player.get_rect()):
            ...
            # Enemigo volador
            self.flying_enemy.update((self.player.x, self.player.y))
            self.flying_enemy.draw(self.screen)

            for projectile in self.flying_enemy.projectiles:
