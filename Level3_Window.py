...
from Intro_Boss import IntroBossWindow
...
class LevelWindow3:
    def __init__(self, screen, clock, gif_path, selected_character, main_game, volume_settings):
        ...
        self.enemies = [
            Enemy(self.start_x + 2 * self.tile_size, self.start_y + 9 * self.tile_size, enemy_sprite_folder, self.tile_size),
            Enemy(self.start_x + 13 * self.tile_size, self.start_y + 1 * self.tile_size, enemy_sprite_folder, self.tile_size),
            Enemy(self.start_x + 7 * self.tile_size, self.start_y + 7 * self.tile_size, enemy_sprite_folder, self.tile_size)            
        ]
        for enemy in self.enemies:
            enemy.active = True
        # Agregado para enemigo volador y proyectiles
        ...
        self.flying_enemies = [
            ...
            FlyingEnemy(self.start_x + 9 * self.tile_size, self.start_y + 5 * self.tile_size, flying_enemy_sprite_folder, self.tile_size)
        ]
        for flying_enemy in self.flying_enemies:
            flying_enemy.active = True
        self.projectiles = []

        # Inicializamos las cajas rompibles a partir del mapa
        self.breakable_boxes = []
        
        # Contamos posiciones de cajas
        cajas_posiciones = []
        ...
        # Elegimos una caja al azar para la llave
        ...
        self.bombs = []
        self.explosions = []
        self.items = []  # Lista para ítems/power-ups

        # Aquí inicializamos las trampas de fuego según el mapa (con 'F')
        self.traps = []
        for row_idx, row in enumerate(self.level_map):
            for col_idx, tile in enumerate(row):
                if tile == 'F':
                    ...
                    trap = Trap(x, y, self.tile_size, trap_type="fire", sprite_base_path=sprite_base_path)
        ...
        self.hud = HUD(self.screen, pygame.time.get_ticks(), lives=self.lives, score=self.score, level=3)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_c:
                        # Mostrar intro del boss antes de continuar
                        if not self.main_game.boss_intro_shown:
                            intro = IntroBossWindow(self.screen, self.clock)
                            intro.run()
                            self.main_game.boss_intro_shown = True
    ...
    def draw_scene(self, move_enemies=True, dt=0):
        ...
        # Dibuja ítems/power-ups
        for item in self.items:
            item.draw(self.screen)

        # Dibujar bolas de fuego activas
        for fireball in self.fireballs[:]:
            result = fireball.update(self.enemies, self.flying_enemies, None, self.screen.get_width(), self.screen.get_height())
...
# Movimiento, disparo y colisiones enemigos voladores
        for f_enemy in self.flying_enemies[:]:
            ...
            f_enemy.shoot()  # Esto actualiza su lista interna de proyectiles

    # Actualizamos los proyectiles del enemigo volador
            for projectile in f_enemy.projectiles[:]:
                projectile.update()

        # Si el proyectil se sale de la pantalla, eliminarlo
                if projectile.is_off_screen():
                    f_enemy.projectiles.remove(projectile)
                    continue

        # Colisión con paredes y cajas irrompibles (bloques "#", cajas "?")
    ...
    # Colisión enemigo volador - jugador
            if f_enemy.get_rect().colliderect(self.player.get_rect()):
                if not self.player.dying and self.player.alive:
                    self.player.lose_life()
                    if self.player.lives <= 0: 
                        result = self.handle_player_death()
    ...
    # Colisión explosiones - enemigo volador
            for explosion in self.explosions:
    ...
    # Dibujar enemigo volador y sus proyectiles
        ...
        for proj in self.projectiles[:]:
            ...
            # Quitar proyectil si sale de pantalla
            if proj.is_off_screen():
                self.projectiles.remove(proj)
                continue

            # Colisiones proyectil - cajas rompibles
            for box in self.breakable_boxes:
                if not box.destroyed and proj_rect.colliderect(box.get_rect()):
                    if proj in self.projectiles:
                        self.projectiles.remove(proj)
                    break
            
            # Colisiones proyectil - bloques irrompibles
            for row_idx, row in enumerate(self.level_map):
                for col_idx, tile in enumerate(row):
                    if tile == "#":
                        block_rect = pygame.Rect(self.start_x + col_idx*self.tile_size, self.start_y + row_idx*self.tile_size, self.tile_size, self.tile_size)
                        if proj_rect.colliderect(block_rect):
                            if proj in self.projectiles:
                                self.projectiles.remove(proj)
                            break
                ...
                while self.running:
                    ...
                    if not self.level_started:
                        if now - self.level_start_time >= self.level_start_duration:
                            pygame.mixer.music.stop()
                            pygame.mixer.music.load(os.path.join("assets", "OST", "Level 3.mp3"))
                        ...
                        if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
                            ...
                            if not self.player.dying and self.player.alive:
                                if self.player.can_place_bomb():
                                    ...
                                    # PASAMOS referencia al jugador en el constructor de la bomba
                    ...
                    # Actualizar cajas rompibles: detectar si fueron destruidas y soltar ítems
                    for box in self.breakable_boxes:
                        if box.destroyed and not box.content_spawned:
                            ...
                            # Actualizar el mapa para que el espacio quede libre
                    ...
                    if self.level_complete:
                        ...
                        if now - self.level_complete_time >= self.level_complete_duration:
                            result = self.show_victory_screen()
